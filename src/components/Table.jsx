...
// Resolves dot-notation paths: "transactions.amount" → row.transactions?.amount
function getVal(row, key) {

  console.log(`GetVal ${row} --- ${key} --- `,key.split(".").reduce((obj, k) => obj?.[k], row));
 ...
 *
 * columns: Array<{
 *   key: string,          — dot-notation path used as React key and fallback cell value
 *   label: string,        — header text
 *   headerClass?: string, — extra class on <th>
 *   className?: string,   — extra class on <td>
 *   render?: (row) => ReactNode — custom cell renderer (receives the full row)
 * }>
 *
                  ...
                  <thead>
                      <tr className="bg-gray-50 border-b border-gray-200 text-left text-gray-500 font-medium">
                          {columns.map((col) => (
                              <th
                                  key={col.key}
                                  className={`px-4 py-3 ${col.headerClass ?? ""}`}
                              >
                                  {col.label}
                              </th>
                          ))}
                                  ...
                                  {columns.map((col) => (
                                      ...
                                      >
                                          {col.render
                                              ? col.render(row)
                                              : getVal(row, col.key)}{" "}
                                          {/* {`Row Value ${row.transactions.transaction_with}`} */}
                                      </td>
                                  ))}
                              </tr>
                          ))
          ...
          {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
                  <p className="text-xs text-gray-500">
                      ...
                      {Math.min(safePage * pageSize, data.length)} of{" "}
                      {data.length}
                  </p>
